...
// Step 2: Twitch redirects here with ?code=...&state=...
twitchOAuthRouter.get('/callback', async (req: Request, res: Response) => {
  const { code, state, error } = req.query;
  const frontendUrl = getFrontendUrl();

  if (error) {
    return res.redirect(`${frontendUrl}?twitch_error=${encodeURIComponent(String(error))}`);
  }

  if (!code || !state) {
    return res.redirect(`${frontendUrl}?twitch_error=missing_params`);
  }
  ...
  try {
    ...
    if (!clientSecret) {
      return res.redirect(`${frontendUrl}?twitch_error=client_secret_not_configured`);
    }
    ...
    if (!tokenRes.ok) {
      ...
      logger.error('Twitch token exchange failed', errText);
    ...
    if (!userRes.ok) {
      return res.redirect(`${frontendUrl}?twitch_error=user_fetch_failed`);
    }

    const userData: any = await userRes.json();
    const twitchUser = userData.data?.[0];
    if (!twitchUser) {
      return res.redirect(`${frontendUrl}?twitch_error=no_user_data`);
    }

    // Save token in oauth: format (compatible with IRC)
