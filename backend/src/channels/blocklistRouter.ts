...
blocklistRouter.get('/:channel', async (req: Request, res: Response) => {
  ...
  const { rows } = await db.query(
    'SELECT id, phrase, created_at FROM channel_blocklist WHERE channel_name=$1 ORDER BY created_at DESC',
    [req.params.channel]
  );
  res.json(rows);
});

blocklistRouter.post('/:channel', async (req: Request, res: Response) => {
  const ok = await userOwnsChannel(req.user?.email, req.params.channel, req.user?.role === 'admin');
  if (!ok) return res.status(403).json({ error: 'not your channel' });
  ...
  try {
    await db.query(
      `INSERT INTO channel_blocklist (channel_name, phrase) VALUES ($1,$2) ON CONFLICT DO NOTHING`,
      [req.params.channel, clean]
    );
    // Reload in engine
    const tm = (global as any).twitchManager;
    if (tm) await tm.reloadBlocklist(req.params.channel);
    res.json({ success: true });
  } catch (err) {
    logger.error('blocklist add error', err);
    res.status(500).json({ error: 'internal' });
  }
});

blocklistRouter.delete('/:channel/:id', async (req: Request, res: Response) => {
  const ok = await userOwnsChannel(req.user?.email, req.params.channel, req.user?.role === 'admin');
  if (!ok) return res.status(403).json({ error: 'not your channel' });
  await db.query('DELETE FROM channel_blocklist WHERE id=$1 AND channel_name=$2', [parseInt(req.params.id), req.params.channel]);
  const tm = (global as any).twitchManager;
  if (tm) await tm.reloadBlocklist(req.params.channel);
  res.json({ success: true });
});
