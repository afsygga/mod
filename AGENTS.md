...
> **📚 Дополнительно — папка [`docs/`](docs/):** навигация «сверху вниз» и
> справочники, которых здесь нет — [обзор архитектуры + диаграмма](docs/architecture.md),
> полный [REST/WebSocket API-референс](docs/api-reference.md),
> [операторский гид](docs/operations.md) (env, деплой, джобы, диагностика) и
> [модель детекта спама](docs/spam-detection.md). Этот файл (`AGENTS.md`) остаётся
> **источником истины** по инвариантам и грабли — доки в `docs/` ссылаются сюда,
> а не переписывают.
...
**Страница `/client_login`** (не источник для системы — токен НЕ хранится).
Отдельная фронт-страница (`pages/ClientLogin.tsx`, роут в `main.tsx`) для выдачи
токена стороннему клиенту (Chatterino). **Implicit-флоу** (`response_type=token`):
бэкенд `GET /api/twitch-oauth/client-connect` редиректит на Twitch authorize с
полным `SCOPES`; токен приходит во ФРАГМЕНТ URL прямо в браузер, на сервер НЕ
попадает и НЕ сохраняется, client_secret не нужен, хрупкую зону рефреша (§8) не
трогает. Страница собирает строку импорта Chatterino
`oauth_token=…;client_id=…;username=…;user_id=…` (client_id из публичного
`/api/auth/config`, username/user_id — client-side Helix `/users`). Требует
регистрации `https://afsyg.gay/client_login` как OAuth Redirect URL в консоли
Twitch. `SCOPES` расширен до полного набора управления каналом/чатом под Chatterino.
...
- **Язык по умолчанию — русский** (`useState<Lang>('ru')`); переключатель RU/EN
  не персистится (каждый заход стартует на RU). Вкладки навбара — через `T[lang]`.
- **Мобилка** (`useIsMobile`, брейкпоинт 768px; медиа `@media (max-width:767px)`
  в `index.css`): на телефоне `backdrop-filter` **выключается** (артефакты), фоны
  сплошные; верхний навбар — горизонтальная лента `.mobile-tabstrip` (скролл, без
  переноса, тапы ≥40px, подпись у активной); сайдбар Admin → горизонтальная лента
  сверху (root `flex-direction:column`); широкие таблицы (Логи) — горизонтальный
  скролл внутри контейнера с общим `min-width` шапки/строк (тело вбок не едет);
  очередь/аналитика/настройки — один столбец, уменьшенные паддинги; консоль команд
  `width: min(380px, 100vw-24px)`; drawer каналов `min(280px,82vw)`.
...
score выше порога) либо оптимизация запроса. См. §23.
...
Чистая логика без БД: `analyze(username, message, suspicion?) → { score, reasons,
similarityPct, whitelistedFlood?, suspicionBonus? }`. Третий аргумент — внешний
сигнал Twitch (§20), передаётся вызывающим; сам движок в БД/сеть не ходит. Профиль на юзера с историей (обрезается по
...
**Блоклист** (`blocklistPhrases`, таблица `channel_blocklist`): жёсткий фильтр,
проверяется ПЕРЕД вайтлистом. Матч по ГРАНИЦЕ СЛОВА на нормализованных токенах
(корректно для кириллицы, с anti-evasion) → `score 100` + `blocklisted:true`.
...
`channel_whitelist`, `twitch_user_meta`, `stream_sessions`,
`stream_game_changes` (§21), `suspicious_users` (§20), `sessions`, `admin_audit`.
...
**Дедуп одного мода на одного юзера (MUTED/AUTO_MUTED):** прежде pile-on-логики
проверяется, мутил ли ЭТОТ ЖЕ `performed_by` этого юзера за последние 6ч. Если да
— прошлая строка **обновляется** на последний мут (`action/duration/created_at`),
новая НЕ создаётся, `mute_count` не растёт (исход `skipped`). Так один нарушитель
не задваивается в статистике мода (в т.ч. случай primary+secondary одним модом).
Pile-on РАЗНЫХ модов сохраняется (проверка по `performed_by`).
...
## 20. Метки подозрительности Twitch (`utils/suspicion.ts`)

Внешний сигнал, которого SpamEngine из текста получить не может: Twitch помечает
аккаунты как обходящие бан / забаненные в связанных каналах. Приходит через
EventSub `channel.suspicious_user.message` и `.update` (скоуп
`moderator:read:suspicious_users` — он уже был в `SCOPES`, просто не
использовался). Подписки едут на ТОМ ЖЕ соединении, что и `channel.moderate` для
канала: тот же мод-токен, те же права. Гранты, выданные до добавления скоупа,
вернут 403 — канал останется без сигнала, детект по тексту не страдает.

Инварианты:

- **Метка — модификатор, а не триггер.** `SpamEngine.analyze()` добавляет очки
  только к УЖЕ ненулевому score. Подозрительный юзер, который пишет нормально, в
  очередь не попадает — иначе метка превращается в теневой бан.
- **Движок остаётся чистой логикой (§14).** Сигнал передаётся третьим аргументом
  `analyze(username, message, suspicion?)`; БД внутри движка по-прежнему нет.
  На горячем пути читается кэш в памяти (`getSuspicionSignal`), не Postgres.
- **Максимум признаков, а не сумма.** Признаки сильно коррелируют (обход бана
  почти всегда идёт с `active_monitoring`), складывать их — наказывать дважды.
  Очки настраиваются через `settings` (`suspicion_points_*`), дефолты:
  restricted 18, ban_evader 18, banned_in_shared 15, active_monitoring 10.
- **Очки меньше разрыва между порогами** (детект 70, автомут 90 → разрыв 20).
  Надбавка кормит ОБА порога, поэтому при больших очках сообщение, не дотянувшее
  до детекта по тексту, могло уйти сразу в автомут — метка Twitch подменяла бы
  собой детект. При максимуме 18 худший случай (69 + 18 = 87) остаётся ниже
  автомута: метка доводит до очереди, решение принимает человек. **Свойство
  ломается, если сузить пороги в настройках или поднять `suspicion_points_*`.**
- **Ручное снятие переживает новые события Twitch.** `cleared_at` не
  сбрасывается при следующем `channel.suspicious_user.message`; данные Twitch
  обновляются, бонус остаётся выключенным. Иначе кнопка «снять» бесполезна.
- `low_trust_status = 'none'` (модератор снял метку в самом Twitch) удаляет
  запись целиком — это уже не «снято вручную», а «сигнала больше нет».

UI: бейдж с крестиком на карточке в очереди модерации; снятие рассылается по WS
(`suspicious_user`), поэтому видно во всех вкладках. API: `GET
/api/moderation/suspicious`, `POST /api/moderation/suspicious/clear`.
Метрики: `afsyg_suspicious_user_events_total{source}`,
`afsyg_suspicion_score_bonus_applied_total`, `afsyg_suspicious_users_tracked{state}`.

## 21. Смены категории внутри стрима (`stream_game_changes`)

`stream_sessions.game` хранит только ТЕКУЩУЮ категорию — поллер перезаписывает
её каждую минуту, поэтому история смен терялась. Теперь каждая смена пишется
отдельной точкой в `stream_game_changes` прямо в `syncStreams()`: перед апсертом
снимается предыдущее значение `game`, после — сравнивается с новым. Первая точка
сессии — стартовая категория с меткой времени начала стрима, а не смена.

- Разрешение = период поллера (60с). Смена, сделанная через `!g`, ловится тем же
  путём — отдельной обработки команды не нужно.
- `NOT EXISTS` в INSERT защищает от дублей подряд идущих одинаковых категорий
  (рестарт бэкенда, повторный тик).
- API `GET /api/streams/:id/games` → `{ started_at, ended_at, changes[] }`.
- На фронте — полоса категорий под графиком стрима + вертикальные маркеры в
  моменты смены + строка в тултипе. Отрезки и цвета считаются по ПОЛНОМУ массиву
  данных, а не по видимому окну: иначе при зуме менялись бы и цвета, и границы
  (см. правило §10 про привязку цвета к сущности).
- WS-событие `game_change` — для живого обновления открытого дашборда.

**Аналитика на этих данных:**
- **Полоса «спам-давления»** на графике стрима (`StreamAreaChart`) — heat по доле
  спама `spam/msgs` за минуту (чисто фронт, данные из `messages-by-minute`).
  Отличает волну спама от просто активного чата.
- **Сравнение категорий** — `GET /api/streams/category-stats?days=&channel=`:
  агрегат эфира/зрителей/спама/модерации по категориям. Атрибуция по СЕГМЕНТАМ
  из `stream_game_changes` (не по `stream_sessions.game`), сегменты одного канала
  не пересекаются во времени → сообщение попадает максимум в один сегмент. UI —
  блок «Категории» в разделе стримов.

## 22. Steam → категория (`steam/SteamSync.ts`)

Стример запускает игру в Steam — категория на канале меняется сама. Опрос
`ISteamUser/GetPlayerSummaries` ОДНИМ запросом на все привязанные каналы (Steam
принимает до 100 steamid). Интервал адаптивный: хоть один привязанный канал в
эфире → каждые 10с (быстрая реакция на запуск/выход из игры), никто не в эфире
→ раз в 60с; на ранних выходах (нет ключа / выключено / нет привязок)
`liveBefore` чистится, чтобы быстрый интервал не залипал. Даже на 10с суточный
лимит Steam (100k) не при делах. Push-уведомлений у Steam нет, только опрос.
`steam_exit_category` при первом старте сеется как `Just Chatting` (ON CONFLICT
DO NOTHING — заданное админом значение, включая пустое, не перетирается).

Инварианты — каждый закрывает конкретный способ сделать хуже:

- **Реагируем на СМЕНУ игры, а не на её наличие.** Если выставлять категорию на
  каждом тике, ручная правка стримером откатывалась бы через минуту. При
  реакции на переход ручная правка живёт до запуска следующей игры.
- **Действуем только когда канал в эфире.** Оффлайн смена запоминается молча
  (`last_game` обновляется), иначе выход в эфир сопровождался бы сменой
  категории на игру, запущенную задолго до стрима. **Исключение:** настройка
  `steam_offline_enabled` (тумблер «Менять категорию и в оффлайне» в админке,
  дефолт выкл) — при вкл оффлайн-смена игры меняет категорию сразу.
- **«В эфире» определяется НЕЗАВИСИМО через Helix `/streams`** (батч до 100,
  app-токен), НЕ через `stream_sessions`/дашборд. Поэтому whitelist
  (`steam_links`) работает и для каналов, которых **нет на дашборде** — без
  рендера стрима и модерации. Helix недоступен → тик пропускается (не меняем
  вслепую).
- **Выход в эфир трактуется как смена** — иначе типовой сценарий «запустил игру
  → пошёл стримить» не поставил бы категорию вообще (игра-то не менялась).
- **`last_game` двигается даже при неудачной смене.** Иначе провал повторялся бы
  каждую минуту, долбя Twitch одним запросом. Причина отказа — в `last_result`,
  видна в админке.
- **Ошибка Steam API не трогает ничего**: ни категорию, ни `last_game`.
- Выход из игры меняет категорию, ТОЛЬКО если задан `steam_exit_category`
  (пусто = не трогать). Иначе альт-табнувшийся стример ронял бы категорию.

Смена идёт через `TwitchManager.setGame()` — тот же путь, что у `!g`, со всем
перебором токенов стримера (§13). Из-за этого `setGame` теперь возвращает
`{ ok, message, category? }`, а не строку: `!g` берёт `message` для чата,
синхронизация — `ok`. Токены нужны те же, отдельной авторизации не требуется.

Настройка — вкладка **Steam** в админке: глобальный выключатель, тумблер
оффлайна, привязки `канал → SteamID64` (строго 17 цифр, vanity-URL не годится) —
это и есть **whitelist автосмены**, поведение при выходе из игры, ручные
соответствия имён (`steam_category_map`), кнопка разовой синхронизации. У каждой
привязки бейдж **АВТОРИЗОВАН / НЕТ ТОКЕНА** (есть ли живой токен стримера —
`users.twitch_oauth` по `twitch_username` или `broadcaster_tokens`); без токена
`setGame` не сработает.

Не лечится кодом: Steam отдаёт игру только при публичной настройке «Данные об
игре», и видит только игры из Steam. Приватный профиль виден в админке как
«Steam не отдаёт профиль». **Пиратки через Steam-эмулятор репортятся как
«Spacewar» (appid 480)** — Steam не различает реальную игру; сейчас НЕ
игнорируется (попытается выставить «Spacewar»). Метрики:
`afsyg_steam_api_requests_total{result}`,
...
Сделано (было в этом списке):

- **Ретенция `messages`** — фоновая джоба `message_retention` в `index.ts`
  (батчами по 5000, раз в 6ч, хранит `spam_score >=` порога). Настройка —
  `MESSAGE_RETENTION_DAYS` / `MESSAGE_RETENTION_KEEP_SCORE`, см.
  [docs/operations.md](docs/operations.md).
- **Кулдаун уведомлений** — per-user окно 60с в `TelegramBot.notifyQueueAdd`;
  гасит только Telegram-пинги, WS-`queue_add` остаётся живым.
- **`Auto-rejoin … failed: undefined`** — исправлено на `err?.message || err`.
